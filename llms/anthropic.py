...
from anthropic.types import ContentBlock, TextBlockParam, ToolParam, ToolUseBlock, ToolUseBlockParam, MessageParam, ToolResultBlockParam
...
def anthropic_tool_call_to_tool_call(tool_call: ToolUseBlock) -> ToolCall:
    tool_call_arguments_dict = tool_call.input.__dict__
    return ToolCall(
...
def message_to_anthropic_message(message: Message) -> MessageParam:
    if not message.content and message.tool_calls:
        return MessageParam(
            content=[
                cast(ContentBlock, ToolUseBlockParam(
                    type="tool_use",
                    name=tool_call.name,
                    id=tool_call.id,
    ...
    if message.role == "user" and message.content:
        return MessageParam(
            content=[
                cast(ContentBlock, TextBlockParam(
                    type="text",
                    text=message.content,
                ))
            ],
...
def get_anthropic_model_response(messages: List[Message], tools: List[Tool], system_prompt: str) -> Message:
    ...
    if message.stop_reason == "tool_use":
        return Message(
            ...
            content=None,
            tool_calls=[
                anthropic_tool_call_to_tool_call(cast(ToolUseBlock, tc)) for tc in message.content
            ]
        )
    ...
    if message.stop_reason == "max_tokens":
        return Message(
            id=message.id,
            role="assistant",
            content=f"Max tokens reached. {anthropic_message_content_to_message_content(message.content)}" if message.content else "Max tokens reached.",
            tool_calls=None
        )
